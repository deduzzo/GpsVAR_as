export default {
	...
	getDataAndCalcolaDifferenze: async () => {
		...
		this.currData = this.generateMapOfMonthData(
	...
	calcolaDifferenze: (currentData, pastData, currentCfToKeys, pastCfToKeys) => {
		...
		for (const key of allKeys) {
			...
			if (cur && prev) {
				...
				// ALTRI CONTROLLI
				//1-VARIABILE GM.PLUS_IC SENZA IND.COORD
				const variabiliKeys = Object.keys(cur.perVariabile)
				// verifico se c'è almeno una chiave che inizia con GM.PLUS_IC
				const hasPlusIC = variabiliKeys.some(k => k.startsWith("GM.PLUS_IC"));

				// verifico se c'è almeno una chiave che inizia con IND.COORD
				const hasIndCoord = variabiliKeys.some(k => k.startsWith("IND.COORD_*"));
				if (hasPlusIC !== hasIndCoord && (rapporto == "GMET" || rapporto == "GMES")) {   // true se ne manca solo una
					let msg = "Anomalia variabili: ";
					if (!hasPlusIC) {
						msg += "manca GM.PLUS_IC_*";
					} else if (!hasIndCoord) {
						msg += "manca IND.COORD_*";
					}
